...
class LockedDropout(nn.Module):
    ...
    It has been shown to be more effective to use the same dropout mask for an entire
    sequence as opposed to a different dropout mask each time.

    Note: reset LockedDropout() layer at the beginning of each batch
    """
    def __init__(self):
...
class RNN_model(nn.Module):
    ...
    def forward(self, x, t, train=True):
        """
        Assume input x is batch_size by sequence_length.
        This means we will be training everything with a fixed sequence length
        We will take care of this during training loop
        """
